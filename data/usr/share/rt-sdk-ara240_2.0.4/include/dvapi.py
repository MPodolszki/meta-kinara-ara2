...
def _dvApiObj() -> CDLL:
    ...
    if curr_platform == "Windows":
        if "DV_TGT_ROOT" in os.environ:
            if os.path.exists(
                os.path.join(env, "art/windows/x86/client/md_release/araclient.dll")
            ):
                lib_loaded = load_dvinfclient_lib(
                    os.path.join(env, "art/windows/x86/client/md_release/araclient.dll")
                )
        else:
            lib_loaded = load_dvinfclient_lib(
                os.path.abspath(os.path.join(cwd, "../client/md_release/araclient.dll"))
            )
    if curr_platform == "Linux":
        ...
        if "x86_64" in os_info[4]:
            if "DV_TGT_ROOT" in os.environ:
                if os.path.exists(
                    os.path.join(env, "art/linux/x86/lib/libaraclient_x86_64.so")
                ):
                    lib_loaded = load_dvinfclient_lib(
                        os.path.join(env, "art/linux/x86/lib/libaraclient_x86_64.so")
                    )
                ...
                lib_loaded = load_dvinfclient_lib(
                    os.path.abspath(os.path.join(cwd, "../lib/libaraclient_x86_64.so"))
                )
        else:
            if "DV_TGT_ROOT" in os.environ:
                if os.path.exists(
                    os.path.join(env, "art/linux/aarch64/lib/libaraclient_aarch64.so")
                ):
                    lib_loaded = load_dvinfclient_lib(
                        os.path.join(
                            env, "art/linux/aarch64/lib/libaraclient_aarch64.so"
                        )
                    )
            else:
                lib_loaded = load_dvinfclient_lib(
                    os.path.abspath(
                        os.path.join(cwd, "/usr/lib/libaraclient_aarch64.so")
                    )
                )

    if not lib_loaded:
        raise DVClientLibNotFoundError(
            errno.ENOENT,
...
###############################################################################
class dv_error_category(IntEnum):
...
class dv_status_code(IntEnum):
    ...
    DV_ENDPOINT_DYN_POWER_SET_FAILURE = (
        506,
    )  # Unable to set dynamic power switch idle time for device
...
class dv_endpoint_state(IntEnum):
    DV_ENDPOINT_STATE_INIT = (
        0,
    )  # endpoint reset done but firmware not loaded yet(this is a transient state)
    DV_ENDPOINT_STATE_IDLE = (
        1,
...
class dv_endpoint_power_state(IntEnum):
    DV_POWER_STATE_L0 = (
        0,
    )  # state with maximum power and performance (default state when endpoint is initialized)
    ...
    DV_POWER_STATE_L2 = (
        3  # endpoint set to a sys clock of 150 MHz and all subsystems are power gated
    )
...
class dv_client_log_level(IntEnum):
    DV_CLIENT_LOG_LEVEL_TRACE = (
        0,
    )  # dump critical, error, warning, info, debug and trace logs
    DV_CLIENT_LOG_LEVEL_DEBUG = (
        1,
    )  # dump critical, error, warning, info and debug logs
    ...
    DV_CLIENT_LOG_LEVEL_OFF = (6,)  # turn of client logs
...
class dv_blob(Structure):
    _fields_ = [
        (
            "handle",
            c_void_p,
        ),  # blob handle (raw pointer or shared file id returned by server)
...
class dv_session(Structure):
    _fields_ = [
        ("handle", c_void_p),  # session private handle, managed by client library
        ("socket_str", c_char_p),  # NULL terminated socket connection string
        ("socket_type", c_int),
    ]  # socket types: Unix domain socket/TCPIPv4


class dv_shm_descriptor(Structure):
    _fields_ = [
        ("handle", c_void_p),  # /**< shared mem handle, managed by client library */
        (
            "session",
            POINTER(dv_session),
        ),  # /**< session handle on which shared memory is registered */
        ("fd", c_int),  # /**< file fd for which shared memory is registered */
        ("size", c_size_t),  # /**< size to map */
        ("offset", c_size_t),  # /**< offset in file */
        ("fd_type", c_int),
    ]  # /**< type of fd shared (reserved) */


class dv_endpoint_chip_info(Structure):
    _fields_ = [
        ("id", c_char_p),  # dv chip id
        ...
        ("l2_memory_size", c_uint),
    ]  # dv chip internal L2 memory size in bytes


class dv_endpoint_dram_info(Structure):
    _fields_ = [
        ("vendor_id", c_uint),  # dv dram vendor id
...
class dv_endpoint_iface_info(Structure):
    ...
    _fields_ = [
        ("type", c_int),  # dv module physical interface (pcie, usb) with host
...
class dv_endpoint_dram_statistics(Structure):
    _fields_ = [
        ...
        (
            ...
            c_uint32,
        ),  # endpoint dram memory occupied by all the active model artefacts in bytes
        ("ep_total_tensor_occupancy_size", c_uint32),
    ]  # endpoint dram memory occupied by all the active model tensors in bytes
...
class dv_inference_queue_statistics(Structure):
    _fields_ = [
        (
            "occupancy_count",
            c_int,
        ),  # Number of inference queue slots occupied with inference request for the endpoint
        ("length", c_int),  # length of the inference queue for the endpoint
        ("wait_time", c_float),
    ]  # waiting time in mili secs for the new inference request to get picked up by endpoint


class dv_model_statistics(Structure):
    _fields_ = [
        ("model", c_uint32),  # model handle
        (
            "active_input_tensors_count",
            c_uint32,
        ),  # number for active model input tensor(s) present in an endpoint
        (
            "active_output_tensors_count",
            c_uint32,
        ),  # number for active model output tensor(s) present in an endpoint
        (
            "active_inferences_count",
            c_uint32,
        ),  # number for active model inference request queued in an endpoint
        (
            "model_total_dram_occupancy_size",
            c_uint32,
        ...
        (
            ...
            c_uint32,
        ),  # total endpoint dram occupancy in bytes by model output tensors
        ("model_handle", c_void_p),
    ]  # since r5.3 void* handle which can be compared to `handle` member of dv_model_t


class dv_endpoint_statistics(Structure):
    _fields_ = [
        ("ep", POINTER(dv_endpoint)),  # endpoint handle
        ...
        (
            "ep_core_voltage",
            c_float,
        ),  # average of endpoint core voltage across all measurement point in hardware in volts
        (
            "ep_temp",
            c_float,
        ),  # average of endpoint temperature across all measurement point in hardware in degree celsius
        (
        ...
        (
            "ep_soft_reset_count",
            c_uint32,
        ),  # endpoint soft reset count, non zero for usb devices
        ("ep_nnp_clk", c_int),  # endpoint nnp clock in MHz
        ("ep_sbp_clk", c_int),
    ]  # endpoint sbp clock in MHz


# Backward compatibility with warning, changing dv_endpoint_stats to dv_endpoint_statistics
class dv_endpoint_stats(dv_endpoint_statistics):
    def __init__(self, *args, **kwargs):
        warnings.warn(
            "'dv_endpoint_stats' is deprecated and will be removed in a future version. Use 'dv_endpoint_statistics' instead.",
            DeprecationWarning,
            stacklevel=2,
        )
...
class dv_model_input_param(Structure):
    _fields_ = [
        ("preprocess_param", POINTER(dv_model_input_preprocess_param)),
        ...
        ("num", c_int),  # num
        ("src_graph_layer_name", c_char_p),
    ]  # src_graph_layer_name


class dv_model_output_postprocess_param(Structure):
    _fields_ = [
        ("qn", c_float),  # output quantization parameter
...
class dv_model_output_param(Structure):
    _fields_ = [
        ("postprocess_param", POINTER(dv_model_output_postprocess_param)),
...
class dv_compiler_statistics(Structure):
    _fields_ = [
        ("config_name", c_char_p),  # DV1 config name, governed on ep system core clock
        ("cycles", c_float),  # total cycles estimated by compiler
        ("ips", c_float),  # inference per seconds estimated by compiler
        ("ddr_bandwidth", c_float),
    ]  # ep dram estimated by compiler
...
class dv_model_load_options(Structure):
    _fields_ = [
        ("model_name", c_char_p),  # model name (unused)
        ("priority", c_int),  # priority of the model
        ("cache", c_bool),  # if true, the model is cached on disk
        (
            "async",
            c_bool,
        ),  # if true, the model load API immediately return \see dv_model_load_wait_for_completion
        (
            "model_type",
            c_int,
        ),  # specify the model type, if not specified it will be  DV_MODEL_TYPE_ARA2_CNN
        (
            "send_model_filepath_to_proxy",
            c_bool,
        ),  # if true, send filepaht from client lib to proxy, imporves host memory consumpion
    ]


# Backward compatibility with warning, changing dv_model_options to dv_model_load_options
class dv_model_options(dv_model_load_options):
    def __init__(self, *args, **kwargs):
        warnings.warn(
            "'dv_model_options' is deprecated and will be removed in a future version. Use 'dv_model_load_options' instead.",
            DeprecationWarning,
            stacklevel=2,
        )
        super().__init__(*args, **kwargs)
...
class dv_product_version(Structure):
    _fields_ = [("product_type", c_int), ("product_version", dv_version)]
...
class dv_stats_options(Structure):
    _fields_ = [("state_type", c_uint32), ("temp_threshold", c_uint32)]
...
class dv_model_llm_params(Structure):
    _fields_ = [
        ...
        ("embedding_lookup_addr", c_uint64),
        ("embedding_lookup_scale_addr", c_uint64),
        ("is_speculative", c_uint32),
        ("max_prompt_input_size", c_uint64),
        ("max_token_input_size", c_uint64),
        ("max_output_size", c_uint64),
        ("is_host_specd", c_uint8),
    ]

    def __str__(self):
        return (
            "dv_model_llm_params <"
            "vocab_size={}, embedding_size={}, input_precision={}, "
            "output_precision={}, max_num_tokens={}, is_dynamic={}, "
            "num_inputs={}, pad_token_id={}, eos_token_id={}, bos_token_id={}, "
            "embedding_lookup_addr={}, embedding_lookup_scale_addr={}, "
            "is_speculative={}, max_prompt_input_size={}, max_token_input_size={}, "
            "max_output_size={}, is_host_specd={}>"
        ).format(
            self.vocab_size,
            self.embedding_size,
            ...
            self.eos_token_id,
            self.bos_token_id,
            self.embedding_lookup_addr,
            self.embedding_lookup_scale_addr,
            self.is_speculative,
            self.max_prompt_input_size,
            self.max_token_input_size,
...
def __str__(self):
    return "dv_model_llm_params <top_k={}, top_p={:.3f}, temperature={:.3f}, repetition_penalty={}, target_token_post_mcp={}, target_token_pre_mcp={}, target_prompt_post_mcp={}, target_prompt_pre_mcp={}, draft_token_post_mcp={}, draft_token_pre_mcp={}, draft_prompt_post_mcp={}, draft_prompt_pre_mcp={}, >".format(
        ...
        self.target_token_post_mcp,
        self.target_token_pre_mcp,
        self.target_prompt_post_mcp,
        self.target_prompt_pre_mcp,
        self.draft_token_post_mcp,
        self.draft_token_pre_mcp,
        self.draft_prompt_post_mcp,
        self.draft_prompt_pre_mcp,
    )


class dv_model(Structure):
    _fields_ = [
        ("handle", c_void_p),  # private handle, managed by client library
        ("session", POINTER(dv_session)),  # session object
...
class dv_infer_statistics(Structure):
    _fields_ = [
        ...
        (
            "input_transfer_time",
            c_float,
        ),  # time taken in mili-seconds to transfer input(s) from host dram to ep hardware dram
        (
            "output_transfer_time",
            c_float,
        ),  # time taken in mili-seconds to transfer output(s) from ep hardware dram to host dram
        (
            "ep_queue_submission_time",
            c_float,
        ),  # time taken in mili-seconds to submit inference request to ep hardw
        ("cumulative_replay_count", c_uint32),
        ("current_replay_count", c_uint32),
        (
            "input_transfer_start_time_stamp",
            c_timespec,
        ),  # time stamp when input transfer started
        (
            "output_transfer_start_time_stamp",
            c_timespec,
        ),  # time stamp when output transfer started
        (
            "inference_start_time_stamp",
            c_timespec,
...
class dv_infer_request(Structure):
    _fields_ = [
        ...
        (
            "ep_submitted",
            POINTER(dv_endpoint),
        ),  # when inference request is queued on group of endpoints, this provide endpoint info on which inference is submitted.
...
class dv_endpoint_hw_statistics(Structure):
    _fields_ = [
        ...
        (
            "ep_core_volt",
            POINTER(c_float),
        ),  # list of endpoint voltage readings for all measurement points in volts
        (
            "num_temp",
            c_int,
        ),  # number of temperature measurement points available in hardware
        ("ep_temp", POINTER(c_float)),
    ]  # list of endpoint temperature readings for all measurement points in degree celsius
...
###############################################################################
def dv_stringify_status_code(status_code: dv_status_code) -> str:
    """
    Parameters
    ----------
    dv_status_code
...
def _glibc_free(ctypes_ptr: c_void_p) -> None:
    fun = _dvApiObj().free
...
def dv_model_set_llm_cfg_params(
    session: dv_session,
    endpoint: dv_endpoint,
    model: dv_model,
    llm_cfg_update: dv_llm_cfg_upd_req,
) -> dv_status_code:
...
def dv_session_create_via_unix_socket(
    socket_path: str,
) -> Tuple[dv_status_code, dv_session]:
...
def dv_session_create_via_tcp_ipv4_socket(
    ipv4_addr: str, port: int
) -> Tuple[dv_status_code, dv_session]:
...
def dv_session_close(session: dv_session) -> dv_status_code:
...
def dv_shmfd_register(
    session: dv_session, fd: int, size: int, offset: int, fd_type: int
):
    ...
    offset : int
        file offset
    fd_type : int
        fd type to provide hint to server (reserved)

    Returns
    -------
    dv_shm_descriptor
        Returns shared memory descriptor if success, else None
    """
    ...
    fun.argtypes = [
        ...
        c_uint32,
        c_int,
        POINTER(POINTER(dv_shm_descriptor)),
    ]
    shm_descriptor = POINTER(dv_shm_descriptor)()
    fun.restype = dv_status_code
    ret = fun(session, fd, size, offset, fd_type, byref(shm_descriptor))
...
def dv_shmfd_unregister(shm_descriptor: dv_shm_descriptor) -> dv_status_code:
    """Un-register shared file with the server
    Parameters
    ----------
    shm_desc : dv_shm_descriptor_t
    ...
    fun.restype = dv_status_code
    return fun(shm_descriptor)


def dv_endpoint_get_list(
    session: dv_session,
) -> Tuple[dv_status_code, dv_endpoint, int]:
...
def dv_model_load_from_blob(session, ep, model_blob, model_name, priority):
    ...
    ret = fun(
        session, ep, model_blob, model_name.encode("utf-8"), priority, byref(model)
    )
...
class dvClassificationOutput(Structure):
    ...
    _fields_ = [("numOutputs", c_uint), ("rows", dvClassificationItem * 10)]
...
def dv_infer_sync(
    session, endpoint, model, inputs, outputs, timeout, enable_stats=True
):
...
def dv_infer_wait_for_completion(session, inf_list, timeout):
    ...
    status = fun(
        session, byref(inf_list), c_int(1), c_int(timeout), byref(completed_req)
    )
...
def dv_infer_get_req_id(infer_req):
    ...
    fun.argtypes = [POINTER(dv_infer_request), POINTER(c_uint64)]
    fun.restype = dv_status_code
    req_id = c_uint()
...
def dv_infer_sync_with_options(
    session: dv_session,
    endpoint: dv_endpoint,
    model: dv_model,
    inputs,
...
def dv_endpoint_free_hw_statistics(ep_hw_stats, count):
    ...
    fun.argtypes = [POINTER(dv_endpoint_hw_statistics), c_int]
    fun.restype = dv_status_code
...
def dv_endpoint_free_dram_statistics(ep_dram_stats, count):
    ...
    fun.argtypes = [POINTER(dv_endpoint_dram_statistics), c_int]
    fun.restype = dv_status_code
    return fun(ep_dram_stats, count)


def dv_endpoint_get_statistics_with_options(
    session: dv_session, ep: dv_endpoint, stats_options: dv_stats_options
):
    """
    Parameters
    ----------
    ...
    stats_options: additional information
         to api
    ...
    """
    fun = _dvApiObj().dv_endpoint_get_statistics_with_options
    fun.argtypes = [
        POINTER(dv_session),
        POINTER(dv_endpoint),
        POINTER(POINTER(dv_endpoint_statistics)),
        POINTER(c_int),
        POINTER(dv_stats_options),
    ]
    fun.restype = dv_status_code
    ep_stats = POINTER(dv_endpoint_statistics)()
    ep_count = c_int()
    status = fun(session, ep, byref(ep_stats), byref(ep_count), stats_options)
...
def dv_endpoint_free_statistics(ep_stats, count):
    ...
    Parameters
    ----------
    ep_stats : dv_endpoint_statistics
    ...
    fun.argtypes = [POINTER(dv_endpoint_statistics), c_int]
    fun.restype = dv_status_code
...
def dv_register_fault_callback(session, fault_handler, user_data=None):
    ...
    dv_endpoint_fault_callback_fun = CFUNCTYPE(
        None, POINTER(dv_endpoint_statistics), c_void_p
    )
...
def dv_model_load_from_file_with_options(
    ...
    ret = fun(session, ep, model_file_path.encode("utf-8"), byref(model), options)
...
def dv_model_load_from_file_with_flags(
    session: dv_session,
    ep: dv_endpoint,
    model_file_path: str,
    flags: int,
...
def dv_model_load_from_blob_with_options(session, ep, blob, options):
    ...
    ret = fun(session, ep, blob, byref(model), options)
...
def dv_exchange_current_version_details(
    session: dv_session,
) -> Tuple[dv_product_version, int]:
    ...
    Returns
    -------
    (dv_product_version, int)
        product_version     : product versions
        product_count       : number of product version sent
    """
    fun = _dvApiObj().dv_exchange_current_version_details
    fun.argtypes = [
        POINTER(dv_session),
        POINTER(POINTER(dv_product_version)),
        POINTER(c_int),
    ]
    fun.restype = dv_status_code
    product_version = POINTER(dv_product_version)()
    product_count = c_int()
    ret = fun(session, byref(product_version), byref(product_count))
    if ret != dv_status_code.DV_SUCCESS:
        return None
    else:
        return product_version, product_count


def dv_retrieve_version_details(session: dv_session) -> Tuple[dv_product_version, int]:
    ...
    (dv_product_version, int)
        ...
        product_count       : number of product version sent
    """
    fun = _dvApiObj().dv_retrieve_version_details
    fun.argtypes = [
        POINTER(dv_session),
        POINTER(POINTER(dv_product_version)),
        POINTER(c_int),
...
def dv_get_client_lib_version() -> dv_version:
    """api gives current running version of client library
    return dv_version in case of success, else returns none

    Returns
    -------
    dv_version
        clientlib_version     : fills struct with client library version
    """
    fun = _dvApiObj().dv_get_client_lib_version
    fun.argtypes = [POINTER(dv_version)]
    fun.restype = dv_status_code
    clientlib_version = dv_version()
    ret = fun(byref(clientlib_version))
    if ret != dv_status_code.DV_SUCCESS:
        return None
    else:
        return clientlib_version


def dv_free_version_details(clientlib_version: dv_product_version) -> dv_status_code:
    """Api deletes memory created by dv_exchange_current_version_details or dv_retrieve_version_details apis"""
    fun = _dvApiObj().dv_free_version_details
    fun.argtypes = [POINTER(dv_product_version)]
    fun.restype = dv_status_code
    return fun(byref(clientlib_version))
...
def dv_model_load_wait_for_completion(model, timeout):
    ...
    fun.argtypes = [POINTER(dv_model), c_int]
...
def dv_model_compare_handles(model1, model2):
    ...
    fun.argtypes = [c_void_p, c_void_p]
    fun.restype = c_bool
...
def dv_get_dynamic_op_size(model, op_blob_list, op_index):
    ...
    status = fun(
        model,
        cast(op_blob_list, POINTER(dv_blob)),
        op_index,
        byref(op_size),
        byref(size_index),
    )
...
def dv_get_endpoint_busyness(session, endpoint):
    ...
    fun.argtypes = [POINTER(dv_session), POINTER(dv_endpoint), POINTER(c_bool)]
    fun.restype = dv_status_code
    is_busy = c_bool()
...
def dv_fetch_outputs_by_layer_name(
    inf_obj: dv_infer_request, src_op_layer_name: str
) -> Tuple[dv_status_code, List[dv_blob]]:
    ...
    fun.argtypes = [
        ...
        c_char_p,
        POINTER(POINTER(dv_blob)),
        POINTER(c_int),
    ]
    fun.restype = dv_status_code

    ret = fun(
        inf_obj, src_op_layer_name.encode("utf-8"), byref(op_blobs), byref(num_op)
    )
    output_blobs = []
    if ret == dv_status_code.DV_SUCCESS:
...
def dv_bulk_read(
    session, endpoint, address, size, buffer, perf_mode=False, completion_time=None
):
...
class DVInferStatistics:
    ...
    def __str__(self):
        return (
            "dvInfStats: HwInfTime={:.2f}ms, HwFpComputationTime={:.2f}ms, HwPerfStats(sclk={} MHz, dclk={} MHz, TotalCycles={}, FPCycles={}), "
            + "ipXferTime={:.2f}ms, opXferTime={:.2f}ms, queueSubmissionTime={:.2f}ms, input_transfer_start_time_stamp={}, output_transfer_start_time_stamp={}, inference_start_time_stamp={}"
        ).format(
...
class DVLLMInferInfo:
    def __init__(self, llm_info: dv_infer_llm_info):
        self.llm_infer_resp_num_valid_tokens = llm_info[
            0
        ].llm_infer_resp_num_valid_tokens
...
class DVInferRequest:
    ...
    def __init__(
        self,
        inputs: List[DVTensor],
        outputs: List[DVTensor],
        inf_req: dv_infer_request,
        model_obj,
    ):
        ...
        if (
            self.status == dv_inference_status.DV_INFERENCE_STATUS_COMPLETED
            or self.status == dv_inference_status.DV_INFERENCE_STATUS_FAILED
        ):
...
class DVModelOutputParam:
    ...
    def __str__(self):
        opparam_str = "layer_id={}, blob_id={}, fused_parent_id={},layer_name={}, blob_name={}, layer_fused_parent_name={}, layer_type={}, layout={}, size={}, width={}, height={}, depth={}, nch={}, bpp={}, num_classes={}, layer_output_type={}, num={}, max_dynamic_id={}, src_graph_layer_name={}, postprocess_param={}".format(
            self.layer_id,
            self.blob_id,
            self.fused_parent_id,
            self.layer_name,
            self.blob_name,
            self.layer_fused_parent_name,
            self.layer_type,
            self.layout,
            self.size,
            self.width,
...
class DVModelLoadOptions:
    ...
    @classmethod
    def model_load_option(
        cls,
        model_name="model.dvm",
        priority=1,
        cache=False,
        model_async=False,
        model_type: dv_model_type = dv_model_type.DV_MODEL_TYPE_ARA2_CNN,
        send_model_filepath_to_proxy: bool = False,
    ):
        m_options = dv_model_load_options(
            model_name.encode("utf-8"),
            c_int(priority),
            c_bool(cache),
            c_bool(model_async),
            dv_model_type(model_type),
            c_bool(send_model_filepath_to_proxy),
        )
        m_options_pointer = pointer(m_options)
        model_load_options = cls(m_options_pointer)
        return model_load_options

    def __str__(self):
        model_load_options = (
            "model_name={}, priority={}, cache={}, async={}, model_type={}".format(
                self.model_name,
                self.priority,
                self.cache,
                self.model_async,
...
class DVLLMModelCfgUpdateParams:
    ...
    def llm_model_cfg_options(
        ...
        draft_token_post_mcp=1,
        draft_token_pre_mcp=1,
        draft_prompt_post_mcp=1,
        draft_prompt_pre_mcp=1,
    ):
...
class DVInferOptions:
    ...
    @classmethod
    def dv_infer_options(
        cls,
        enable_stats: bool,
        infer_type: dv_infer_type,
        active_tokens: int,
        valid_tokens: int,
        tokens_to_skip: int,
    ):
...
class DVModel:
    ...
    def _allocate_output_tensors(self):
        ...
        for idx, (offset, tensor_name) in enumerate(offsets):
            tensor_slice = output_tensor_contig[start_offset : offset + start_offset]

            slices[idx] = DVTensor(tensor_slice, self.output_param[idx])
            start_offset += offset
        return slices
    ...
    def _output_tensors_to_blobs(self, output_tensors):
        ...
        for idx, param in enumerate(self.output_param):
            ...
            if tensor.mem_desc is not None:
                output_tensor_blobs[idx].for_shm_desc(
                    tensor.mem_desc._shm_desc, tensor.offset, param.size
                )
            else:
    ...
    def _input_tensors_to_blobs(self, input_tensors):
        ...
        for idx, param in enumerate(self.input_param):
            ...
            if tensor.mem_desc is not None:
                input_tensor_blobs[idx].for_shm_desc(
                    tensor.mem_desc._shm_desc, tensor.offset, param.size
                )
    ...
    def infer_sync(
        self, input_tensors, output_tensors=None, endpoint=None, timeout=50000
    ):
    ...
    def infer_sync_with_options(
        self,
        input_tensors: List[DVTensor],
        output_tensors: List[DVTensor],
        endpoint: dv_endpoint,
        timeout_ms: int,
        infer_options: DVInferOptions,
    ):
        def get_blobs_frm_tensors_list(tensors: List[DVTensor]):
        ...
        status, inf_req_obj = dv_infer_sync_with_options(
            self.session,
            self.endpoint,
            self._model,
            input_tensor_blobs,
    ...
    def infer_async(self, input_tensors, output_tensors=None, endpoint=None, desc=None):
        ...
        status, inf_req_obj = dv_infer_async(
            self.session,
            self.endpoint,
            self._model,
            input_tensor_blobs,
    ...
    def infer_sync_with_flags(
        self,
        input_tensors: List[DVTensor],
        output_tensors: List[DVTensor] = None,
        flags=None,
        timeout=60000,
        endpoint=None,
        desc=None,
        ...
        status, inf_req_obj = dv_infer_sync_with_flags(
            self.session,
            self.endpoint,
            self._model,
            input_tensor_blobs,
...
class DVCompilerStats:
    ...
    def __str__(self):
        compiler_stats_str = (
            "config_name={}, cycles={}, ips={}, ddr_bandwidth={}".format(
...
class DVEndpointChipInfo:
    ...
    def __str__(self):
        chip_info_str = "id={}, rev={}, control_processor_count={}, neural_processor_count={}, l2_memory_size={}".format(
            self.id,
            self.rev,
            self.control_processor_count,
            self.neural_processor_count,
            self.l2_memory_size,
        )
...
class DVSession:
    ...
    def get_loaded_endpoint_list(self, model):
        ...
        (
            ...
            endpt_list_size,
            loaded_endpt_idxes,
            loaded_endpt_list_size,
        ) = dv_model_get_loaded_endpoint_list(self._session, model._model)
        if ret != dv_status_code.DV_SUCCESS:
            return ret, None, None, None, None
    ...
    def load_model_from_file(
        ...
        model_path,
        model_name="model.dvm",
        priority=dv_model_priority_level.DV_MODEL_PRIORITY_LEVEL_DEFAULT,
        cache_model=False,
    ):
        ...
        -------
        (dv_status_code, DVModel)
            DV_SUCCESS, DVModel object if load call succeeded on all devices
            DV_PARTIAL_SUCCESS, DVModel object if model is loaded only on some devices
            DV_MODEL_LOAD_FAILURE, DVModel object if model load failed on all devices
        ...
        if cache_model:
            ...
            ret, model_handle = dv_model_load_from_file(
                self._session, endpoint._endpoint, model_path, model_name, priority
            )
    ...
    def load_model_from_file_with_flags(
        self,
        endpoint,
        model_path,
        flags,
        ...
        """
        ret, model_handle = dv_model_load_from_file_with_flags(
            self._session,
            endpoint._endpoint,
            model_path,
            flags,
    ...
    def load_model_from_file_with_options(
        ...
        """
        ret, model_handle = dv_model_load_from_file_with_options(
            self._session, endpoint._endpoint, model_path, options._model_load_options
        )
