...
if [[ -f /run/ara240-bootstrap ]]; then
	echo "[ ADD ] $UPTIME: Service is bootstrapping. Skipping restart." >>/var/log/udev_ara2_rules.log
	exit 0
fi
...
elif [[ "$(systemctl is-enabled rt-sdk-ara2)" == "enabled" ]]; then
	last_count=0
else
	echo "[ ADD ] $UPTIME: Reference count not found. Please be sure file ${COUNT_FILE} exist" >>/var/log/udev_ara2_rules.log
	exit 0
fi

# If count changed, restart the service
if [[ "$total" -ne "$last_count" ]]; then
	echo "[ ADD ] $UPTIME: Changes on device count restarting curr.: $total, prev: $last_count." >>/var/log/udev_ara2_rules.log
	systemctl restart rt-sdk-ara2.service
else
	echo "[ ADD ] $UPTIME: Device count still the same..." >>/var/log/udev_ara2_rules.log
fi
