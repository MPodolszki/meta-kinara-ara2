...
class dv_shm_descriptor(Structure):
    _fields_ = [ ("handle", c_void_p),                                 # /**< shared mem handle, managed by client library */
                 ("session", POINTER(dv_session)),                     # /**< session handle on which shared memory is registered */
...
class dv_endpoint_statistics(Structure):
    _fields_ = [("ep",                              POINTER(dv_endpoint)),                          # endpoint handle
                ...
                ("ep_sbp_clk",                      c_int)]                                         # endpoint sbp clock in MHz                                      
...
class dv_model_load_options(Structure):
     _fields_ = [("model_name",c_char_p),                  # model name (unused)
                 ...
                 ("model_type", c_int),                    # specify the model type, if not specified it will be  DV_MODEL_TYPE_ARA2_CNN
                 ("send_model_filepath_to_proxy",c_bool),  # if true, send filepaht from client lib to proxy, imporves host memory consumpion
                 ]
...
class dv_product_version(Structure):
    _fields_ = [("product_type", c_int),
                ("product_version", dv_version)]
...
class dv_model(Structure):
    _fields_ = [("handle",              c_void_p),                              # private handle, managed by client library
                ...
                ("model_load_options",  POINTER(dv_model_load_options)),             # @since r6.0 options used when loading the model
                ("cp_layer",            c_bool)]                        
...
def dv_session_create_via_unix_socket(socket_path : str)-> Tuple[dv_status_code, dv_session]:
...
def dv_session_create_via_tcp_ipv4_socket(ipv4_addr : str, port: int)-> Tuple[dv_status_code, dv_session]:
...
def dv_shmfd_register(session: dv_session, fd: int, size: int, offset:int, fd_type:int) -> dv_shm_descriptor | None:
...
def dv_endpoint_get_list(session : dv_session) -> Tuple[dv_status_code, dv_endpoint, int]:
...
def dv_client_set_log_level(log_level):
    ...
    fun.argtypes = [dv_client_log_level]
    fun.restype = dv_status_code
...
def dv_endpoint_free_statistics(ep_stats,count):
    ...
    fun.argtypes = [POINTER(dv_endpoint_statistics),
                    c_int]
    fun.restype = dv_status_code
...
def dv_register_fault_callback(session, fault_handler, user_data=None):
    ...
    """
    dv_endpoint_fault_callback_fun  = CFUNCTYPE(None, POINTER(dv_endpoint_statistics), c_void_p)
...
def dv_model_load_from_file_with_options(session : dv_session, ep : dv_endpoint, model_file_path : str, options: dv_model_load_options):
...
def dv_exchange_current_version_details(session : dv_session) -> Tuple[dv_product_version, int]:
...
def dv_free_version_details(clientlib_version : dv_version) -> dv_status_code:
...
def dv_fetch_outputs_by_layer_name(inf_obj : dv_infer_request, src_op_layer_name : str) -> Tuple[dv_status_code, List[dv_blob]]:
...
class DVInferRequest:
    ...
    def __init__(self, inputs : List[DVTensor], outputs : List[DVTensor], inf_req : dv_infer_request, model_obj):
    ...
    def fetch_outputs_for_layer(self, src_op_layer_name : str) -> Tuple[dv_status_code, List[dv_blob]]:
...
class DVModelLoadOptions:
    ...
    @classmethod
    def model_load_option(cls, model_name = "model.dvm", priority = 1, cache = False, model_async = False, model_type : dv_model_type = dv_model_type.DV_MODEL_TYPE_ARA2_CNN, send_model_filepath_to_proxy: bool = False):
        m_options = dv_model_load_options(model_name.encode('utf-8'), c_int(priority), c_bool(cache), c_bool(model_async), dv_model_type(model_type), c_bool(send_model_filepath_to_proxy))
...
class DVModel:
    ...
    def infer_sync_with_options(self, input_tensors : List[DVTensor], output_tensors : List[DVTensor], endpoint : dv_endpoint, timeout_ms : int, infer_options : DVInferOptions):
    ...
    def infer_sync_with_flags(self, input_tensors : List[DVTensor] , output_tensors : List[DVTensor]= None, flags = None, timeout = 60000, endpoint=None, desc=None ):
        if output_tensors is None:
            output_tensors = self._allocate_output_tensors()
